...
describe 'sending sms reminders', :integration do
  ...
  context 'for reminders' do
    ...
    context 'with a reminder due more than 10 minutes ago' do
      before do
    ...
    context 'with a reminder due in the future' do
      before do
  ...
  context 'for events' do
    ...
    let(:event_list1) { create(:event_list, :with_account) }
    let(:event_list2) { create(:event_list, account: account) }
    let(:event1) { create(:event, account: account, event_list: event_list1, due_at: 10.minutes.ago) }
    let(:subscriber1) { create(:subscriber, account: account, phone_number: '15555555555') }

    context 'with an event due in the previous 10 minutes' do
      let(:subscriber2) { create(:subscriber, account: account, phone_number: '15555555556') }
      let(:event2) { create(:event, account: account, event_list: event_list2, due_at: 9.minutes.ago) }
    ...
    context 'with an event due more than 10 minutes ago' do
      before do
        event1.update_attributes!(due_at: 11.minutes.ago)
      end
    ...
    context 'with an event due in the future' do
      before do
        event1.update_attributes!(due_at: 1.minute.from_now)
      end
